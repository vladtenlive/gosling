
# GOsling - real gopher docs and a real hero

## Check wiki first

[Home · golang/go Wiki (github.com)](https://github.com/golang/go/wiki)

## Ready to Go?

[A Tour of Go](https://go.dev/tour/list)

[How to Write Go Code - The Go Programming Language](https://go.dev/doc/code)

[Effective Go - The Go Programming Language](https://go.dev/doc/effective_go)

[GopherCon 2014 Closing Day Keynote by Andrew Gerrand - YouTube](https://www.youtube.com/watch?v=dKGmK_Z1Zl0)

## Practice! You can't learn kung fu by watching kung fu!

[Go on Exercism](https://exercism.org/tracks/go)

[CodeCrafters | Advanced programming challenges](https://codecrafters.io/)

## Read the spec! Source code is the key

[The Go Programming Language Specification - The Go Programming Language](https://go.dev/ref/spec)

[golang/go: The Go programming language (github.com)](https://github.com/golang/go)

## Security

[OWASP/Go-SCP: Go programming language secure coding practices guide (github.com)](https://github.com/OWASP/Go-SCP)

## Bloggers

[Go (Golang) Programming Blog - Ardan Labs](https://www.ardanlabs.com/blog/)

[A Journey With Go – Medium](https://medium.com/a-journey-with-go)

[Three Dots Labs - start here](https://threedots.tech/start/)

[Applied Go · Applied Go](https://appliedgo.net/)

[Go — Bitfield Consulting](https://bitfieldconsulting.com/golang)

## It's a TRAP

[Martin Baillie | Gotchas in the Go Network Packages Defaults](https://martin.baillie.id/wrote/gotchas-in-the-go-network-packages-defaults/)

[Darker Corners of Go – Rytis Biel](https://rytisbiel.com/2021/03/06/darker-corners-of-go/)

[Common Gotchas in Go - 0xDEADBEEF](https://deadbeef.me/2018/01/go-gotchas)

[Faster Go code by being mindful of memory | Food for thought (f4t.dev)](https://f4t.dev/software/go-performance-memory/)

[Do you make these Go coding mistakes? · YourBasic Go](https://yourbasic.org/golang/gotcha/)

[50 Shades of Go: Traps, Gotchas, and Common Mistakes for New Golang Devs (cloudimmunity.com)](http://devs.cloudimmunity.com/gotchas-and-common-mistakes-in-go-golang/)

[How to avoid Go gotchas · divan's blog](https://divan.dev/posts/avoid_gotchas/)

## Keep learning

[Go 101](https://go101.org/)

[Algorithms to Go · YourBasic](https://yourbasic.org/)

[Practical Go Lessons (practical-go-lessons.com)](https://www.practical-go-lessons.com/)

[Go by Example](https://gobyexample.com/)

[Learn Go with Tests - Learn Go with tests (gitbook.io)](https://quii.gitbook.io/learn-go-with-tests/)

## GOing deep

[go-internals - Go Internals (gitbook.io)](https://cmc.gitbook.io/go-internals/)

[emluque/golang-internals-resources: A collection of articles and videos to understand Golang internals. (github.com)](https://github.com/emluque/golang-internals-resources)

[The Go Memory Model - The Go Programming Language](https://go.dev/ref/mem)

[LearnConcurrency · golang/go Wiki (github.com)](https://github.com/golang/go/wiki/LearnConcurrency)

## Add some Style

[How to Write Go Code - The Go Programming Language](https://go.dev/doc/code)

[guide/style.md at master · uber-go/guide (github.com)](https://github.com/uber-go/guide/blob/master/style.md)

[CodeReviewComments · golang/go Wiki (github.com)](https://github.com/golang/go/wiki/CodeReviewComments)

## DIY

[Building a BitTorrent client from the ground up in Go | Jesse Li (jse.li)](https://blog.jse.li/posts/torrent/)

[codecrafters-io/build-your-own-x: Master programming by recreating your favorite technologies from scratch. (github.com)](https://github.com/codecrafters-io/build-your-own-x)

[Visualize your local Git contributions with Go (flaviocopes.com)](https://flaviocopes.com/go-git-contributions/)

[Build a Command Line app with Go: lolcat (flaviocopes.com)](https://flaviocopes.com/go-tutorial-lolcat/)

[Building a CLI command with Go: cowsay (flaviocopes.com)](https://flaviocopes.com/go-tutorial-cowsay/)

[Containers From Scratch • Liz Rice • GOTO 2018 - YouTube](https://www.youtube.com/watch?v=8fi7uSYlOdc)

[Lexical Scanning in Go - Rob Pike - YouTube](https://www.youtube.com/watch?v=HxaD_trXwRE)

[Let's Create a Simple Load Balancer With Go - kasvith.me](https://kasvith.me/posts/lets-create-a-simple-lb-go/)

## Open Source

[Pion](https://pion.ly/)

[Watermill](https://watermill.io/)

## Databases

[Go database/sql tutorial (go-database-sql.org)](http://go-database-sql.org/index.html)

## MIT Distributed Systems course using Go

[6.5840 Home Page: Spring 2023 (mit.edu)](https://pdos.csail.mit.edu/6.824/)


